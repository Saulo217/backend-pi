<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>dicas</title>
    <link rel="manifest" href="http://localhost/backend-pi/manifest.json">
    <link rel="stylesheet" type="text/css" href="../css/admin.css">
    <style>
        @font-face {
            font-family: "Montserrat-Bold";
            src: url("../assets/fonts/Montserrat/Montserrat-Bold.ttf") format("truetype");
        }
    </style>
</head>
<body>
    <header><button onclick="goToPage('admin_app')"><</button></header>
    <section class="container-body">
        <article class="title-page">
            <p>Teste</p>
        </article>
        <article class="container-descricao">
            <p class="descricao">Boa noite meu nome é Marcelo, utilizo este software desde o lançamento e acho muito bom e prático porém desde ontem meu app está um pouco travado.</p>
        </article>
        <button class="btn-resolvido">Entrar</button>
    </section>



    <footer><img src="../assets/footer_background.png" alt="footer_background" /></footer>
</body>
</html>
